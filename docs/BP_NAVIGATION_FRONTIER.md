...
`RMQBPNavigation` exposes neutral names under `RMQ.BPNavigation`. It is a
narrow spoke: close-position LCA navigation over Cartesian-shape
balanced-parentheses encodings, not yet a complete tree-navigation library.
...
succinct RMQ. The useful next deepening steps are:
