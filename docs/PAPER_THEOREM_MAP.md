...
Its one typed 22-source manifest over logical segments `0..22` includes
canonical close; BP roles `0` and `19` share one physical source, and it covers every
...
arbitrary-result predicate, and fresh segment 23 fails that common relation.
...
literal bound `210` in one checked type. No payload padding manufactures
equality.
...
principled `210` charged-trace bound transfers, and invalid ranges return `none`.
...
`SuccinctFinal.concreteBPNativeSuccinctRMQPrincipledAllSizeChargedTraceCost = 210`.
The checked sum is `2*35 + (2*11 + 2*37 + 33) + 11`. On the actual canonical
trace, every event is proved to be `readWord` by
...
same execution before being bounded by `210`. That equality uses the canonical
...
This accepted-route accounting charges payload reads only.
