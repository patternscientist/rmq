...
- [`PROJECT_STATE_2026_06_28.md`](PROJECT_STATE_2026_06_28.md): current
  ...
  rank-slack milestones, with branch-relative notes for the latest
  rank/select primary-budget and union-find Tarjan-level proof worktrees.
...
- [`RANK_SELECT_FID_FRONTIER.md`](RANK_SELECT_FID_FRONTIER.md): the rank/select
  ...
  charged reads, narrow metadata accounting, and the branch-relative
  log-chunk primary block-code budget bridge.
- [`UNION_FIND_AMORTIZATION_FRONTIER.md`](UNION_FIND_AMORTIZATION_FRONTIER.md):
  the union-find spoke after the rank-bucket, rank-slack, and branch-relative
  Tarjan-level scaffold milestones, and why inverse-Ackermann accounting is
  still open.
