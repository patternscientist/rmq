...
publication-facing RMQ theorem surface on
`codex/rmq-u3-principled-allsize-cost` after the U3 charged-trace repair. Dated
digests are source-history artifacts, not competing current summaries. When
...
The document-role manifest at
[`../internal/PUBLICATION_DOCUMENT_ROLES.json`](../internal/PUBLICATION_DOCUMENT_ROLES.json)
is the machine-readable source for current-public, compatibility,
audit-evidence, and exact frozen-snapshot classifications.
...
The topology and claim checks are intentionally proposition-aware. Resolving a
documentary theorem name, or proving that an old spelling disappeared, does
not by itself prove that surrounding prose identifies the current payload,
execution, bound, and model boundary.
