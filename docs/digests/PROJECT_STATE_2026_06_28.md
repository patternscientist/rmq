...
Status label: the stable base is `main` at `c92c8af`. Two proof worktrees have
newer branch-local results:

- rank/select worktree `codex/rank-select-compressed-fid-capstone`, with the
  log-chunk primary fixed-weight budget bridge;
- union-find worktree `codex/union-find-tarjan-levels`, with the first
  Tarjan-level potential scaffold.

Until those proof branches merge, statements about those two updates are
branch-relative and provisional. After merge, remove this warning and treat the
same theorem handles as ordinary project state.
...
Branch-relative rank/select update: the compressed/FID path has now removed a
major counting blocker for sentinel log chunks. The new proof worktree proves
...
Branch-relative union-find update: the Tarjan-level worktree adds a multilevel
...
inverse-Ackermann schedule.

## What Became Stale

The previous digest was accurate for `main` at `c92c8af`, but stale relative
to the two proof worktrees in three ways.
...
primary fixed-weight budget bridge is now proved in the rank/select proof
worktree. The remaining blocker is concrete family instantiation.
...
worktree adds a Tarjan-level scaffold splitting cross-level gaps from residual
within-level slack.
...
The new branch-local result proves that splitting into log chunks does not
...
a potential function pays for the pointer chasing. The new branch-local
Tarjan-level scaffold is exactly about choosing a better potential function.
...
No. The branch-local primary budget bridge removes an important counting
