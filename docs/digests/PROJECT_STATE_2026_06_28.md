...
| Union-find | Parent, rank, and mass fields are executable representation data in the forest model. | Invariants such as `RootMassInvariant`, `RankPowerMassInvariant`, and potential-drop proofs. | `Costed` trace length and `RepresentationAmortizedBackend` inequalities. | Not a mutable-array implementation or compiled Lean execution theorem. |
