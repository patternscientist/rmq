...
Therefore `207` is not a conventional word-RAM theorem, a
...
paper-facing bound is the decomposed `207` theorem above.
...
- The current concrete BP-native RMQ paper-facing all-size query-cost bound is
  the uniform canonical charged-trace constant `207`; controller operations are
