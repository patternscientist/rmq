...
`33` on the charged six-span cross-macro route. A canonical reachable witness
at size `3469` attains exactly `33` and rejects `cost <= 30` on the same object.
...
weight zero. The strong current theorem
`RMQ.Headlines.succinctRMQWholeQueryGlobalWordTraceResultReadWordOnly` proves
that every emitted event is `readWord`; the trace has no synthetic marker, and
therefore that the certificate-weight sum equals both trace length and the
`Costed` cost of the same execution before being bounded by `210`.
...
typed 22-source universe over logical segments `0..22` includes canonical
close; BP roles `0` and `19` share one physical source, source regions are
exclusive, live segment `21` is present, segment `23` has no producer,
...
The provenance layer separates two quantifier domains. Its per-query theorem starts from a
...
segment `23` fails `Q`. Earlier event-value and component may-read facts remain
compatibility facts.
...
compatibility history, not in this current reviewer-route explanation. The current cost proof's
paper-facing bound is the decomposed `210` theorem above. The public
`canonicalTransitionalQueryCost = 328` identity is literal-pinned history;
the separately named live raw-expression compatibility value is `352`.
...
`SuccinctClose.ConcreteCompactBPCloseLCADirectory`: the two-level
...
- `RMQ/Core/SuccinctFinalRAM.lean`:
  ...
  `SuccinctFinal.concreteBPNativeSuccinctRMQWholeQueryGlobalWordTraceCostedWithStore_cost_le_of_footprint_global_liveCompatibility`,
...
- `RMQ/Core/SuccinctRMQClassic.lean` (12):
  ...
  `SuccinctClassic.queryCosted_cost_le_liveCompatibility`,
