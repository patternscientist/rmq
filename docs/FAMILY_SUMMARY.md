...
target-independent global payload-store execution story
...
| Shape and microtable core | Shape/RMQ behavior equivalence, exact fixed-size shape signatures, recursive canonical representative arrays, theorem-backed stack/right-spine shape construction, shape universe count, certified raw local microtable, exact in-block backend. | Raw shape lookup cost bounded by `blockSize + 1`; shape count bounded by Catalan envelope `shapeCount b <= 4^b`. | The local theorem is now consumed by `Impl.FischerHeun`; representative arrays provide concrete shape witnesses for lower-bound interfaces, and `SuccinctClassic.prepareInput` uses `Cartesian.stackCartesianShape_eq_shape` to route prepared executable shape construction through the stack/right-spine builder without changing reference semantics. |
...
payload store.  `RankSelect.compressedFIDFixedWeightGlobalPayloadStore_execution_story`
then relabels shared access plus rank false/true and select false/true packets
into one concrete target-independent payload store.
The bounded companion
`RankSelect.compressedFIDFixedWeightGlobalPayloadStore_bounded_execution_story`
...
the declared trace-local width.  The earlier target-indexed global store
...
- `RMQ/Core/Shape.lean`: `CartesianShape`, `CartesianShape.size`,
  ...
  `addConst`, `StackCartesianTree`, `stackCartesianShape`,
  `CartesianShape.representative`, `ShapeOfSize`,
...
- `RMQ/Core/Shape.lean` (43): `shapeRange_size`, `shape_size`,
  ...
  `shapeRange_addConst`, `shape_addConst`, `stackCartesianShape_eq_shape`,
...
- `RMQ/Core/SuccinctFinalRAM.lean`:
  ...
  `SuccinctFinal.concreteBPNativeSuccinctRMQWholeQueryGlobalWordTraceResultWithStore_successful_reads_backed_by_counted_flat_payload_of_footprint_global`,
...
- `RMQ/Core/RankSelectCompressedSubLogRAM.lean` and
  ...
  `RankSelectSpec.subLogCompressedFIDGlobalPayloadStore_execution_story`,
  `RankSelectSpec.subLogCompressedFIDGlobalPayloadStore_bounded_execution_story`,
  ...
  `RankSelect.compressedFIDFixedWeightGlobalPayloadStore_execution_story`,
  `RankSelect.compressedFIDFixedWeightGlobalPayloadStore_bounded_execution_story`,
...
- `RMQ/Core/SuccinctRMQClassic.lean` (12):
