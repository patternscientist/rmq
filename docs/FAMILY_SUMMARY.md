...
```text
2 * select13 + (2 * rank4 + 2 * endpointFringe37 + interior30) + rank4 = 142
```
...
`Costed` cost of the same execution before being bounded by `142`.
...
Therefore `142` is not a conventional word-RAM theorem, a
...
paper-facing bound is the decomposed `142` theorem above.
...
- The current concrete BP-native RMQ paper-facing all-size query-cost bound is
  the uniform canonical charged-trace constant `142`; controller operations are
