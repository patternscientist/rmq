...
| --- | --- | --- | --- | --- |
| Uniform all-size canonical global trace has principled charged-trace cost `76`, with exact cost equal to emitted trace length. | `RMQ.Headlines.succinctRMQWholeQueryGlobalWordTraceCostedCostLe`, `RMQ.Headlines.succinctRMQWholeQueryGlobalWordTraceCostedCostEqTraceLength`, `RMQ.Headlines.succinctRMQQueryCostEq` | `RMQ.SuccinctFinal.concreteBPNativeSuccinctRMQWholeQueryGlobalWordTraceCosted_cost_le_principledAllSizeChargedTrace`, `...cost_eq_trace_length`, `...PrincipledAllSizeChargedTraceCost_eq` | `RMQ/Core/SuccinctFinalRAM.lean` | `lake env lean scripts/headline_axiom_check.lean` |
| The named cost algebra is `2*select13 + (2*rank4 + 2*fringe4 + interior30) + rank4`; charged and zero-weight controller operations are inventoried for E1. | `RMQ.Headlines.succinctRMQChargedTraceCostAlgebra`, `RMQ.Headlines.succinctRMQChargedTraceOperations`, `RMQ.Headlines.succinctRMQCurrentlyUnchargedControllerOperations` | `RMQ.SuccinctFinal.CanonicalRMQChargedTraceCostAlgebra`, `canonicalRMQCurrentTraceWeight` and checked weight inventories | `RMQ/Core/SuccinctFinalRAM.lean` | `lake build RMQPaper` |
