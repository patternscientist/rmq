...
| Agreement on the first physical execution's consumed ordered footprint determines the complete physical execution; a disagreement at a consumed address makes the executions unequal. | `RMQ.Headlines.succinctRMQReviewerPhysicalExecutionEqOfOrderedFootprint`, `RMQ.Headlines.succinctRMQReviewerPhysicalExecutionNeOfConsumedReadDisagreement` | `RMQ.SuccinctFinal.concreteBPNativeSuccinctRMQWholeQueryFlatPhysicalTraceResultWithStore_eq_of_orderedFootprint`, `RMQ.SuccinctFinal.concreteBPNativeSuccinctRMQWholeQueryFlatPhysicalTraceResultWithStore_ne_of_consumed_read_disagreement` | `RMQ/Core/SuccinctFinalStoreParam.lean` | `lake env lean scripts/wordram_axiom_check.lean`; `lake env lean scripts/headline_axiom_check.lean` |
| One exhaustive typed 20-source universe includes canonical close; counted source iff reviewer-live source, source regions are exclusive, all logical segments are covered, each counted source has a named consumer or is shared BP code, emitted reads translate into listed physical storage, and legacy duplicate close/interior slots are absent from the canonical payload. | Construction-facing typed manifest theorems | `RMQ.SuccinctFinal.concreteBPNativeSuccinctRMQReviewerPhysicalSources_all`, `...ReviewerSource_counted_iff_live`, `...ReviewerPhysicalSources_nodup`, `...ReviewerPhysicalSources_length`, `...ReviewerSource_counted_consumer_or_sharedBP`, `...ReviewerSource_region_injective`, `...ReviewerSegmentSource?_coverage`, `...ReviewerSegmentRegion?_coverage`, `...ReviewerPhysicalSources_exclude_legacy_close`, `...CanonicalReviewerReadStore_legacyTail_none`, `...GlobalReadStore_eq_reviewerPhysical` | `RMQ/Core/SuccinctFinal/RAM/ReviewerPhysical.lean` | `lake env lean scripts/wordram_axiom_check.lean` |
