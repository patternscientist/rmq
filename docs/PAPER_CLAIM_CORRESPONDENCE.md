...
| One exhaustive typed 20-source universe includes canonical close; counted source iff operationally live source, source regions are exclusive, all logical segments are covered, emitted reads reach counted evaluator leaves, every counted source reaches an actual read-producing evaluator branch, all select/rank/canonical-close shared-BP dependencies are checked, and legacy duplicate close/interior slots are absent. | `RMQ.Headlines.succinctRMQReviewerEveryReadOperationalSource`, `RMQ.Headlines.succinctRMQReviewerCountedSourceEvaluatorConnection`, `RMQ.Headlines.succinctRMQReviewerSharedBPConsumerEvaluatorConnection` | `RMQ.SuccinctFinal.concreteBPNativeSuccinctRMQReviewerSource_counted_iff_live`, `...WholeQueryGlobalWordTraceResult_read_operational_source`, `...ReviewerSource_counted_evaluator_connection`, `...ReviewerSharedBPConsumer_evaluator_connection`, `...ReviewerPhysicalSources_nodup`, `...ReviewerSource_region_injective`, `...ReviewerSegmentSource?_coverage`, `...ReviewerPhysicalSources_exclude_legacy_close`, `...CanonicalReviewerReadStore_legacyTail_none` | `RMQ/Core/SuccinctFinal/RAM/ReviewerPhysical.lean`; `RMQ/Core/SuccinctFinalRAM.lean` | `lake env lean scripts/wordram_axiom_check.lean`; `lake env lean scripts/headline_axiom_check.lean` |
