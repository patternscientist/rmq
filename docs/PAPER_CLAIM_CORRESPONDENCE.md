...
| Paper theorem / claim row | Lean alias | Source theorem | Source file | Exact check command |
| --- | --- | --- | --- | --- |
| Clean all-size query-cost statement: the final global trace cost is bounded by the route-split all-size expression and by the fixed modeled constant `65585`; the legacy `196727` aggregate remains checked only as compatibility. | `RMQ.Headlines.succinctRMQWholeQueryGlobalWordTraceRouteSplitCostedCostLe`, `RMQ.Headlines.succinctRMQWholeQueryGlobalWordTraceCostedCostLe`, `RMQ.Headlines.succinctRMQQueryCostEq`, and `RMQ.Headlines.succinctRMQLegacy196727QueryCostEq` | `RMQ.SuccinctFinal.concreteBPNativeSuccinctRMQWholeQueryGlobalWordTraceCosted_cost_le_routeSplit`, `RMQ.SuccinctFinal.concreteBPNativeSuccinctRMQWholeQueryGlobalWordTraceCosted_cost_le_cleanAllSize`, `RMQ.SuccinctFinal.concreteBPNativeSuccinctRMQCleanAllSizeQueryCost_eq`, and `RMQ.SuccinctFinal.concreteBPNativeSuccinctRMQQueryCost_eq` | `RMQ/Core/SuccinctFinal.lean`; `RMQ/Core/SuccinctFinalRAM.lean`; `RMQ/Headlines/RMQ.lean` | `lake env lean scripts/headline_axiom_check.lean` |
