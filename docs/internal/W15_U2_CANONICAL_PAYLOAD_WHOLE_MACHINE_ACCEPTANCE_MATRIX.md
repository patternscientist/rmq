...
| ID | Exact check | Required outcome | Evidence obtained | Status |
| --- | --- | --- | --- | --- |
| `POLICY-R-CHK-01` | `claim_drift_policy_regression.ps1` | All category-level must-reject, must-accept, allowance-bypass, path-context, and parser-shape fixtures receive the expected production final strict verdict. | Pending. | Open. |
| `POLICY-R-CHK-02` | `claim_drift_scan.ps1 -Strict` | Exit 0 with zero unapproved strict failures in the repository. | Pending. | Open. |
| `POLICY-R-CHK-03` | focused absolute-Windows-path scanner regression | A drive-qualified single-file misuse fails, and absolute scanning of the marked matrix uses the same checked contextual allowance as its relative path. | Pending. | Open. |
| `POLICY-R-CHK-04` | `design_decision_check.ps1 -Base c3c3b51b216c9c01be20dca46e8acfc6872e7d57 -Strict` | Exit 0. | Pending. | Open. |
| `POLICY-R-CHK-05` | `git diff --check` | Exit 0. | Pending. | Open. |
| `POLICY-R-CHK-06` | full `scripts/gate.ps1` | Exit 0 after the production-verdict regression and repository strict scan; `GATE PASS` present. | Pending. | Open. |
| `POLICY-R-CHK-07` | verify blocking CI configuration | Repository gate, strict claim scan, and the chosen strict design-decision disposition are blocking, not advisory. | Pending. | Open. |
| `POLICY-R-CHK-08` | verify clean local/remote matching HEAD | After the final ledger commit and push, the worktree is clean and local/remote branch SHAs are identical. | Pending. | Open. |
