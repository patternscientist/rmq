...
| `EG-CP-F03` evidence | **Amendment 2026-07-26, post-A10.** The requirement text is unchanged. The minimum evidence becomes a **universal checked congruence over the whole executed controller**: for all inputs of equal `n`, all endpoints, and any two stores agreeing on the *ordered read footprint*, the emitted trace and the returned value are identical — together with anti-vacuity witnesses (dropped-hypothesis controls must fail; the footprint hypothesis must admit stores that differ; the transcript must demonstrably vary with `n`, with endpoints, and with probe replies). | The exhaustive typed inventory served two purposes: (a) model faithfulness — nothing observable depends on input content beyond `n`; and (b) syntactic eliminability — no content-dependent computation remains in the controller's text. The congruence establishes (a) directly and universally, and is strictly better evidence for it than an enumeration whose completeness must itself be argued. It does **not** establish (b): a content-dependent intermediate can be dead or can cancel before any emitted event (A10-F03-01). Purpose (b) is **not deleted** — it is already `EG-CP-F06`'s "closed signature", and F06's own minimum evidence already reads "cross-shape transcript determinism for equal allowed inputs/probe replies". The inventory was doing F06's work inside F03. `EG-CP-F04`/`F06` may not be recorded closed by citing this amendment. |
| `EG-CP-F03` header words | **Amendment 2026-07-26, post-A10.** F03 may be discharged with the store as an undifferentiated read oracle, provided `EG-CP-F01`/`F02` then show that the frozen header fields are exactly the non-probe metadata the controller consumes, at which point F03 is re-checked against the typed header. | The requirement names header words as a permitted input, but no header exists until F01 freezes the schema, so the obligation is not dischargeable in F03's own time slot (A10-F03-04). This defers it with a named back-reference rather than silently absorbing header words into an opaque store. Binding the supplied store to `memory xs` of the model contract at :327-338 likewise belongs to `EG-CP-F01`/`F04`. |
