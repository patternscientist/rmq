...
The positive certificate at the amended width is hypothesis-free:
`wholeQueryProgram_fits_reviewerWordBits` (`E1WholeQueryPathWidth.lean:470`) —
no size threshold, no parametric `w`, no premise about interior, select or
composition.
...
because it records why this amendment was needed, and
`programSkeleton_not_fits_machineWordBits` (`E1ReviewerWidth.lean:399`) is a
