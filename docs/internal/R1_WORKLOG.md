...
| `INV-HISTORICAL-PRESERVATION` | Only new R1-R4 worklog and DD-20260719-006 evidence was appended; prior worklog/decision bodies and all dated/historical paths are unchanged. | Material preservation closed. |
...
| `design_decision_check.ps1 -Strict -Base 263d004f...` | Same repaired tree with the decision recorded on the worker branch as DD-20260719-003 and remapped at integration to DD-20260719-006 | 6.1 s, exit 0, 9 changed files checked. |
...
| `INV-HISTORICAL-PRESERVATION` | All dated digests, prior audits, matrices, earlier worklog bodies, and earlier decision bodies are unchanged. R1-R4 and DD-20260719-006 are append-only. | Closed. |
...
### R1-R4 integration note

The accepted worker branch originally numbered its strong-attribution decision
`DD-20260719-003`. The independently developed B7-R3 lineage already uses that
identifier in the integrated frontier. The integration therefore remaps only
the R1-R4 decision identifier to `DD-20260719-006`; the accepted decision body,
theorem attribution, and worker evidence are unchanged. References to modeled
cost `207` in this worklog describe the exact historical R1-R4 candidate. The
integrated current public surfaces retain the later charged B7 cost `210` and
the same strong readWord-only theorem attribution.

### R1-R4 plus B7-R4 integration repair

The accepted R1 lineage and accepted B7-R4 lineage were siblings. Their first
combined build exposed two omitted, proof-only declarations in
`ReviewerReachabilitySmall`: the explicit-instruction occurrence receipt and
the singleton same-block segment-21 occurrence theorem consumed by semantic
adequacy. Both declarations existed on the accepted R1 branch, but an automatic
overlap resolution had retained the newer B7-R4 occurrence block at the same
location while accepting downstream R1 consumers. Integration restores the two
R1 declarations additively; no evaluator, payload, store, trace, cost, or
public query definition changes.

| Integration row | Exact obligation | Evidence and disposition |
| --- | --- | --- |
| `INT-R1B7-RECEIPT-API` | Preserve the R1 explicit-instruction occurrence receipt while retaining B7-R4's literal `0`/`15`, instruction `0`/`1` proposition. | `ReviewerReadOccurrenceReceiptAtInstruction` and `reviewerReadOccurrenceReceiptAtInstruction_of_producer` are restored before the R1 repeated-read consumers. The focused `ReviewerReachabilitySmall` build passed in 13.8 s; the B7-R4 exact-position theorem and headline consumer remained in the successful root build. |
| `INT-R1B7-SAMEBLOCK-CONSUMER` | Preserve the R1 same-block segment-21 occurrence identity consumed by `ConcreteBPNativeSuccinctRMQReviewerManifestSemanticAdequacy`. | `concreteBPNativeSuccinctRMQSingleton_sameBlockFringeChunk_indexed_occurrence_receipt` is restored with the identical whole-query producer, instruction `2`, folded state, invocation, local position, structural trace, and charged subtrace. The focused semantic-adequacy build passed in 25.1 s. |
| `INT-R1B7-CURRENT-CLAIMS` | Do not regress the later B7 cost/source/freshness/position facts or the R1-R4 strong theorem attribution. | Strict claim scan passed with 1,044 hits and zero failures; registered surfaces retain cost `210`, logical segments `0..22`, live `21`/fresh rejected `23`, positions `0`/`15` from instructions `0`/`1`, and `succinctRMQWholeQueryGlobalWordTraceResultReadWordOnly`. |
| `INT-R1B7-PUBLIC-CONSUMERS` | The combined source and public roots elaborate with the curated trust inventories. | Final `lake build RMQ RMQPaper RMQExamples` passed in 47.6 s. `wordram_axiom_check.lean` passed in 125.0 s and `headline_axiom_check.lean` passed in 50.6 s with only the allowed `propext`, `Classical.choice`, and `Quot.sound` trust base. |

The first aggregate attempt failed in 36.8 s at the missing receipt API. After
that focused repair, the next aggregate attempt failed in 59.5 s at the missing
same-block theorem. The focused semantic repair then passed, followed by the
single final aggregate certification. No failed aggregate was repeated on an
unchanged tree.
