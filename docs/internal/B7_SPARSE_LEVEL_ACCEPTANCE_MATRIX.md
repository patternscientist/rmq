...
| `INV-B7R3-CHARGED-LEVEL-PRESERVATION` | No payload/store/cost/source definition changed. Exact `33`/not-`<=30`, charged table, one-word width, source/segment, `218 -> 527`, little-o, and exact `210` surfaces remain in the curated builds/trust inventories. | Evidence complete subject to final exact-commit certification. |
...
| `REPLAY-SUBPROCESS-DEADLINE` | Positive deadlines were used; startup, selector, full registry, and negative controls all returned before deadline. No survivor or retry existed. The topology lint's sandboxed dependency-download failure received no semantic verdict and its permitted rerun passed. | Evidence complete; final clean check pending. |
| `CHK-B7R4-FOCUSED-BUILDS` | Both defining modules, Classic, Headlines, validation, examples, and both trust files passed development builds; exact timings are in `B7_WORKLOG.md`. | Evidence complete subject to required post-commit direct and single aggregate certification. |
| `CHK-B7R4-TRUST` | Both curated trust inventories passed with literal expected consumers. Topology and both strict DD baselines pass; working diff check passes; both hygiene scans and added-line scan have no hits. | Evidence complete subject to post-commit rerun/range/clean checks. |
...
| `COMPLETE-B7R4-COMMITTED-EVIDENCE` | Intended paths contain the two source propositions, guarded public/headline surfaces, independent expected consumers, DD-20260719-005, this matrix, and the worklog/digestion packet. | Pending intentional stage, one commit, exact committed-range checks, and clean status. |
...
script, and unrelated A07 validator were not run. Coordinator acceptance,
document-authority consolidation, stale-prose disposition, and `STRETCH-01`
remain outside this candidate rung.
