...
Campaign branch `claude/b1-b2-charged-fringe-tables`, HEAD `790d7b3`.
...
| 2 | `...LocalSpanCandidateComputation` | 2311 | 1, then `some`→#1 / `none`→pure | owed |
| 3 | `...GlobalSpanCandidateComputation` | 2329 | 1, then `some`→#1 / `none`→pure | owed |
...
| Fact | Anchor |
|---|---|
| composed 177-instruction interior leg | `summaryMinCandidate_runsTo` — `E1InteriorMinCandidate.lean:929` |
| its receipt = the route's read log | `summaryMachineTrace_eq_routeReads` — `:1200` |
| min-candidate receipt | `minCandidateMachineTrace_eq_routeReads` — `:1259` |
...
| **read-free 3-way merge, already built** | `candMerge3` — `E1CandMerge3.lean:198`, `candMerge3_readFree` `:206` |
...
| interior fold preservation clause | `ChunkFoldUntouched` `E1InteriorChunkFold.lean:928`, clause at `:1011` |
...
Summary+min-candidate `105..117`. **Next free block opens at `118`.**
...
- **Prose only, deferred three times as concern-mixing** — fold in only if
  already editing that file: `E1InteriorChunkStore.lean:31` cites
  `probeShape_unbounded_agreement_fails`; the theorem is
  `unbounded_agreement_refuted` (`:537`).
