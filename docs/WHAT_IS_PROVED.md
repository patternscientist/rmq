...
one exhaustive typed 22-source universe over logical segments `0..22`, with BP
roles `0` and `19` sharing one physical source and canonical close included.
Live segment `21` is present; segment `23` has no producer. Physical
...
principled all-size charged-trace bound is `210`, checked by
...
| `RMQ.Headlines.listIntSuccinctRMQPaperMainTheorem` | Paper-facing theorem combining the manifest packet and existing payload/answer/provenance facts with the guarded reviewer-native certificate, its independent 24-field required-facts projection, guarded list packet, literal same-execution weighted-trace bound `<= 210`, and exact ordered-read complete-`TraceResult` agreement. |
...
| `RMQ.Headlines.listIntSuccinctRMQQueryCostedWithStoreEqQueryCostedOfFootprint` | List-facing safe-footprint equality derived through agreement on the first execution's ordered dynamic reads and complete trace-result equality before projection to `Costed`. |
...
| `RMQ.Headlines.succinctRMQCanonicalReviewerPayloadGlobalWordTraceTwoSidedProfile` | Canonical construction-facing capstone: doubled-Catalan space envelopes, the at-most `2*n + o(n)` canonical reviewer payload, exact physical erasure, direct positional physical backing for every successful read, exact canonical-global-trace answers, non-synthetic certificate weight equal to trace length and the same `Costed.cost`, and uniform bound `210`. |
| `RMQ.Headlines.succinctRMQGlobalPayloadStoreExecutionStory` | All-size final-query execution story: the public costed query refines one globally segmented `WordRAM.TraceEvent` stream and every read agrees with one concrete payload store. The strong event statement is the separate readWord-only alias below. |
...
| `RMQ.Headlines.succinctRMQWholeQueryGlobalWordTraceCostedCostLe` | The unchanged uniform canonical final trace has the principled charged-trace bound `210`. |
...
| `RMQ.Headlines.succinctRMQWholeQueryGlobalWordTraceResultReadWordOnly` | Strong current theorem: every actual emitted canonical event is `readWord`. |
...
| `RMQ.Headlines.succinctRMQWholeQueryGlobalWordTraceResultNonSyntheticWeightSumLe210` | The non-synthetic-weighted actual emitted trace is at most `210`. |
...
| `RMQ.Headlines.succinctRMQReviewerMachineWellFormed` | Named 24-field certificate over the current payload, 22-source physical store, exact trace/footprint/backing/controller objects, direct `<= 210` same-trace bound, and reviewer-width facts. |
| `RMQ.Headlines.succinctRMQReviewerMachineRequiredFacts` | Independent typed 24-proposition consumer filled only by literal projections from the certificate. |
| `RMQ.Headlines.listIntSuccinctRMQReviewerNativeMachineAdequacy` | Guarded ordinary-list packet connecting the list query, canonical costed trace, interpreted controller, exact supplied physical execution, and coherent invalid semantics. |
...
| `RMQ.Headlines.succinctRMQWholeQueryGlobalWordTraceResultWithStoreEqGlobalOfFootprint` | Safe final-layout agreement first yields exact ordered dynamic-read agreement; complete `TraceResult` determinism then identifies the supplied replay with the canonical trace. |
...
the principled charged-trace all-size cap `210`. The exact public
...
route-dependent source description with one typed 22-source universe over
logical segments `0..22`. It includes canonical close, lists each named
consumer (with BP roles `0` and `19` explicitly sharing one physical source),
...
sum equals the same `Costed.cost` and is at most `210`. The answer-rank leaf is a
