...
| `RMQ.Headlines.succinctRMQFlatPayloadStoreNoSyntheticExecutionStory` | Flat-payload no-synthetic execution story: every actual successful read in the final query trace has source/component/offset evidence in one query-independent counted flat layout; the finite-small interior branch is confined to non-Ready shapes below the named `2^15` Ready threshold. |
