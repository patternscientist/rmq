...
The W18 producer-provenance repair candidate preserves W17's exhaustive typed
...
Every emitted read now retains its actual producing instruction, actual
prefix-fold state, physical source, logical segment, and concrete component
path. Every counted source has a concrete may-read path, each shared-BP
consumer has a same-event path, and fresh unused segment `21` is rejected
because no instruction trace can produce it.
