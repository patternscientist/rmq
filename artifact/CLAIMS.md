...
| The uniform canonical reviewer trace has the principled charged-trace bound `76`. Every actual emitted event is `readWord`, `wordRank`, or `wordSelect`; its direct `WordRAM.TraceEvent.chargedWeight` sum equals both trace length and the `Costed` cost of the same execution, and is at most `76`. | `RMQ.Headlines.succinctRMQWholeQueryGlobalWordTraceResultEventReadWordOrWordRankOrWordSelect`, `RMQ.Headlines.succinctRMQWholeQueryGlobalWordTraceResultSyntheticCostOnlyPrimitiveNotMem`, `RMQ.Headlines.succinctRMQWholeQueryGlobalWordTraceResultChargedWeightSumEqTraceLength`, `RMQ.Headlines.succinctRMQWholeQueryGlobalWordTraceResultChargedWeightSumEqCost`, `RMQ.Headlines.succinctRMQWholeQueryGlobalWordTraceResultChargedWeightSumLe76` | Corresponding `RMQ.SuccinctFinal.concreteBPNativeSuccinctRMQWholeQueryGlobalWordTraceResult_*` theorems; `RMQ.WordRAM.TraceEvent.chargedWeight` | `lake env lean scripts/headline_axiom_check.lean` and `lake env lean scripts/wordram_axiom_check.lean` |
