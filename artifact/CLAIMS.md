...
| The ordinary `List Int` supplied-store surface runs `SuccinctClassic.queryCostedWithStore` against a caller-provided store agreeing with `SuccinctClassic.globalReadStore xs` on the final checked footprint: the supplied-store query equals canonical `queryCosted`, valid windows are exact, the all-size modeled cost transfers, and the Ready-threshold fast-regime cost transfers. | `RMQ.Headlines.listIntSuccinctRMQQueryCostedWithStoreEqQueryCostedOfFootprint`, `RMQ.Headlines.listIntSuccinctRMQFinalFullModelSoundnessExactOfFootprintGlobal`, `RMQ.Headlines.listIntSuccinctRMQFinalFullModelCostLeOfFootprintGlobal`, `RMQ.Headlines.listIntSuccinctRMQFastRegimeFinalFullModelCostLeOfFootprintGlobal` | `RMQ.SuccinctClassic.queryCostedWithStore_eq_queryCosted_of_footprint`, `RMQ.SuccinctClassic.listIntFinalFullModelSoundnessExactOfFootprintGlobal`, `RMQ.SuccinctClassic.listIntFinalFullModelCostLeOfFootprintGlobal`, `RMQ.SuccinctClassic.listIntFastRegimeFinalFullModelCostLeOfFootprintGlobal` | `lake build RMQPaper` and `lake env lean scripts/headline_axiom_check.lean` |
...
| The final globally segmented BP-native RMQ trace has a route-split all-size cost theorem and a clean fixed all-size bound `65585`, with the old `196727` aggregate retained only as legacy compatibility. | `RMQ.Headlines.succinctRMQWholeQueryGlobalWordTraceRouteSplitCostedCostLe`, `RMQ.Headlines.succinctRMQWholeQueryGlobalWordTraceCostedCostLe`, `RMQ.Headlines.succinctRMQQueryCostEq`, `RMQ.Headlines.succinctRMQLegacy196727QueryCostEq` | `RMQ.SuccinctFinal.concreteBPNativeSuccinctRMQWholeQueryGlobalWordTraceCosted_cost_le_routeSplit`, `RMQ.SuccinctFinal.concreteBPNativeSuccinctRMQWholeQueryGlobalWordTraceCosted_cost_le_cleanAllSize`, `RMQ.SuccinctFinal.concreteBPNativeSuccinctRMQCleanAllSizeQueryCost_eq`, `RMQ.SuccinctFinal.concreteBPNativeSuccinctRMQQueryCost_eq` | `lake env lean scripts/headline_axiom_check.lean` |
...
- The current concrete BP-native all-size query-cost surface is route-split,
  with a clean fixed corollary `65585`. The separate fast-regime theorem under
