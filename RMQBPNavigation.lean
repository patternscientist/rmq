...
over Cartesian-shape BP encodings plus close-position LCA navigation, with
exactness and payload/word-bound profiles exposed through `RMQ.BPNavigation`.

The public bridge/profile theorems are
`RMQ.BPNavigation.shapeAccessCloseRankProfile` and
