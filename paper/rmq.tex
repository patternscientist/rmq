...
% Ledger anchors: \ledger{ID} ties the preceding mathematical claim to the
% row ID in paper/THEOREM_LEDGER.md. The macro renders nothing; the checker
...
\texttt{1490c97b399d136bad4e18953441da433d130d4d}. It is not a submission,
...
cell-probe strengthening is under separate feasibility review; this draft
reserves exactly one marked insertion point for that result and asserts
nothing about it. Novelty statements are deliberately conditional pending a
...
\texttt{1490c97b399d136bad4e18953441da433d130d4d}; the ledger
...
word-RAM model (Section~\ref{sec:quantities}). The current space theorem
counts advertised payload bits, not the allocated capacity of a packed
memory object (Section~\ref{sec:quantities}); an allocated-capacity
strengthening is exactly what the pending architecture work targets
...
\item[Allocated bits.] The full capacity of a concrete cell array,
  including any header, final-cell padding, sentinel or dead cells. The
  current mainline theorems do \emph{not} bound this quantity; the
  physical-erasure theorem of Section~\ref{sec:model-adequacy} equates
  payload \emph{bit contents} with the flattened word list, which is
  deliberately weaker than an allocated-capacity bound. Bounding allocated
  capacity by $2n + o(n)$ is part of the pending architecture target
  (Section~\ref{sec:pending}).\ledger{L-OPEN-03}
...
bit array with a frozen width function. That packaging is precisely the
pending architecture target of Section~\ref{sec:pending}. Charging an
...
\texttt{1490c97b399d136bad4e18953441da433d130d4d}. The artifact reproduction
path is:
...
\begin{targetstmt}[Frozen target; not a result of this draft]
\label{tgt:packed}
...
At the base commit this target is a \emph{feasibility candidate} under an
active gate: the geometry-closure and controller-closure rows are being
discharged or refuted on a separate governed branch, and neither a positive
architecture acceptance nor a checked obstruction has been recorded. This
draft therefore asserts nothing about the target beyond its statement, and
the repository's status labels for it are reproduced faithfully in the
theorem ledger as \textsc{provisional\_architecture}.

\subsection{Result insertion point}\label{sec:insertion}

Exactly one location in this manuscript may receive the packed all-size
result if and when it is independently accepted; until then the marker
below is the entire content of that location, and no other section may
state, assume, or paraphrase the pending result as established.

\medskip
\noindent\verb|ARCHITECTURE_RESULT_PENDING|
\medskip

\noindent If the gate instead records a checked obstruction or an
unresolved pivot, this section will record that exact outcome, and the rest
of the draft stands unchanged: every theorem above is already checked at
the base commit and none depends on the pending target.
...
whose model vocabulary the pending target of Section~\ref{sec:pending}
follows.
...
\begin{enumerate}
\item \emph{Allocated capacity.} No current theorem bounds the full
  allocated capacity of a single packed memory object (header, padding,
  sentinels included) by $2n + o(n)$; the payload theorem counts advertised
  payload bits, and the physical-erasure theorem equates bit contents only.
  This is the pending architecture target.\ledger{L-OPEN-03}
...
lower bound in matching integer arithmetic. The deliberate residue --- an
allocated-capacity theorem over one packed object with a closed controller
--- is frozen as a target with an active feasibility gate, and this draft
will absorb exactly one result at the marked insertion point when and only
when that work is independently accepted.
