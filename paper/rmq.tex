...
not yet absorbed into this draft; we reserve one marked insertion point for
it, and state it only there and in the allocated-bits entry of
Section~\ref{sec:quantities}. Novelty statements are deliberately
...
repository at the base commit, but is not yet absorbed into a theorem
environment here. This draft states it in exactly two places: the
...
result \emph{as a theorem of this draft}. The allocated-bits entry of
Section~\ref{sec:quantities} also states the capacity bound, with an explicit
ledger citation rather than as a theorem here; no section other than that
entry and this one states, assumes, or paraphrases the result. Two consequences of the acceptance are already reflected
...
that location. Outside it, only the allocated-bits entry of
Section~\ref{sec:quantities} states the capacity bound, and it does so with an
explicit ledger citation rather than as a theorem of this draft.
