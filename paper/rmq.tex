...
% The packed all-size architecture result is stated as a theorem in Section 9,
% and as a capacity bound with a ledger citation in the allocated-bits entry.
...
% `targetstmt` quoted the packed architecture result as a provisional target.
% Section 9 now states it as a theorem (RC-4, 2026-08-16), so this environment
% has no uses. Kept defined so a future provisional target gets the same
% presentation instead of a reinvented one.
...
\texttt{0665b494707695a70675fef0e5c8682f4d80fe0c}. It is not a submission,
...
object and the attempted probes into it, is stated as a theorem in
Section~\ref{sec:pending} and, as a capacity bound, in the allocated-bits
entry of Section~\ref{sec:quantities}. It appears nowhere else. Novelty statements are deliberately
conditional pending a completed search log.
\end{abstract}
...
\texttt{0665b494707695a70675fef0e5c8682f4d80fe0c}; the ledger
...
different object, which this draft states only in the allocated-bits entry of
Section~\ref{sec:quantities} and in Section~\ref{sec:pending}. Priority claims are deliberately conditional:
...
Kazunari Tanaka~\cite{AGQT19}, and the search log
...
counted-header cell-probe refinement --- has been \emph{accepted} in the
repository and is stated as a theorem here. This draft states it in
exactly two places: the
allocated-bits entry of Section~\ref{sec:quantities}, which cites its ledger
row, and Section~\ref{sec:pending}. It asserts it nowhere else.
...
\texttt{0665b494707695a70675fef0e5c8682f4d80fe0c}. The artifact reproduction
path is:
...
\item \texttt{powershell -File paper/check\_paper.ps1} --- the
  ...
  duplicate labels and keys, forbidden tokens, theorem-ledger coverage,
  evidence-matrix status vocabulary, source-citation resolution, and
  \emph{at most one} pending-architecture insertion point --- there are
  currently none, the result having been absorbed into Section~9);
...
\section{A packed cell-probe architecture}
\label{sec:pending}
...
usually read~\cite{GalMiltersen07,LiuYu20}. The repository's endgame roadmap
freezes such a target:

\begin{theorem}[Packed cell-probe architecture]
\label{thm:packed}
...
index after at most $427$ \emph{attempted} adaptive probes into that same
array.\ledger{L-ARCH-01}
\end{theorem}

\noindent The statement is machine-checked as a $39$-field certificate,
inhabited for every input list and every endpoint pair; one import of the
paper root supplies it. Three readings it does \emph{not} license, each of
which the repository states explicitly:

\begin{itemize}
\item $427$ bounds \emph{attempted} probes and is an upper bound, not an
      attainment claim. No execution is known to reach it; the pinned
      fixture issues $68$.
\item The model is cell-probe, so this is not word-RAM instruction time, not
      preprocessing time, and not measured runtime. Computation between
      probes is free.
\item The $210$ inside $427 = 1 + 2\cdot 3 + 2\cdot 210$ is the packed
      controller's own structural countdown. It is \emph{not} the
      charged-trace budget of Section~\ref{sec:cost}, and the two are
      independent at declaration and proof-term level --- a property the
      repository checks rather than asserts.
\end{itemize}
...
The editorial step has now been taken: Theorem~\ref{thm:packed} above states
the accepted result as a theorem of this draft, at the level the rest of the
manuscript uses. The allocated-bits entry of Section~\ref{sec:quantities}
also states the capacity bound, with an explicit ledger citation; no section
other than that entry and this one states, assumes, or paraphrases the
result.

\subsection{Result location}\label{sec:insertion}

Exactly one location in this manuscript carries the packed all-size result,
and it is Theorem~\ref{thm:packed} above. Outside it, only the allocated-bits
entry of Section~\ref{sec:quantities} states the capacity bound, and it does
so with an explicit ledger citation rather than as a theorem of this draft.

The pending marker that previously stood here has been removed, because the
result it reserved space for is now stated. It was retained through earlier
drafts so the manuscript checker could confirm the result appeared in exactly
one place; that check now permits zero markers and fails only on more than
one, so an absorbed result no longer has to advertise itself as unabsorbed.
...
Bender--Farach-Colton~\cite{BFC00}. Fischer and Heun~\cite{FischerHeun07}
gave the systematic $2n + o(n) + \len{A}$-bit constant-time scheme; the
non-systematic (encoding) $2n + o(n)$-bit constant-time scheme, which
answers without consulting the array, is Fischer and
Heun's~\cite[Thm.~5.8, Cor.~5.9]{FischerHeun11}, with
Fischer~\cite{Fischer10} its preliminary LATIN~2010 version. The earliest
non-systematic succinct RMQ is Sadakane's $4n + o(n)$ balanced-parentheses
encoding of the Cartesian tree with an $o(n)$-bit LCA computation
therein~\cite{Sadakane07}, whose setting --- though not whose mechanism ---
the construction of Section~\ref{sec:architecture} shares.
...
attaining decoder. The substantially stronger cell-probe redundancy lower bounds
...
structures''~\cite{TAG16}; Affeldt, Garrigue, Qi and Tanaka record a
similar priority belief, scoped to their LOUDS and dynamic-bit-vector
developments~\cite{AGQT19}. Accordingly, this draft claims no priority for
mechanized succinct-structure verification. The present development
...
closed controller --- has since been accepted in the repository and is
stated as Theorem~\ref{thm:packed}, and only there. What
