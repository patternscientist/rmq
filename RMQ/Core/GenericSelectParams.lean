...
`n` directly (the BP original `canonicalSparseDenseFalseSelectOverhead` fed
`2 * n` only because its argument was `shape.size` and the bit length is
`2 * size`).
-/
def canonicalOverhead (n : Nat) : Nat :=
  SuccinctSelectProposal.sparseDenseFalseSelectOverhead 32 32 32 32 n

theorem canonicalOverhead_littleO :
    SuccinctSpace.LittleOLinear canonicalOverhead := by
  unfold canonicalOverhead
  exact SuccinctSelectProposal.sparseDenseFalseSelectOverhead_littleO 32 32 32 32
