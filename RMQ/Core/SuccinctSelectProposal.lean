...
def builtRectangularFalseSelectSuperEntry
  ...
  basePosition :=
    (RMQ.Succinct.select false shape.bpCode
      (superSlot * sparseDenseFalseSelectSuperStride shape)).getD
      shape.bpCode.length
  spanClass := 0
  pointer := 0

def builtRectangularFalseSelectSuperEntries
    (shape : Cartesian.CartesianShape) :
    List SparseDenseFalseSelectLocatorEntry :=
  (List.range (builtRectangularFalseSelectSuperSlotCount shape)).map
    (builtRectangularFalseSelectSuperEntry shape)
...
def builtRectangularFalseSelectLocalEntry
    (shape : Cartesian.CartesianShape) (globalLocalSlot : Nat) :
    SparseDenseFalseSelectLocatorEntry where
  baseOccurrence :=
    builtRectangularFalseSelectLocalBaseOccurrence
  ...
  basePosition :=
    (RMQ.Succinct.select false shape.bpCode
      (builtRectangularFalseSelectLocalBaseOccurrence
        shape globalLocalSlot)).getD shape.bpCode.length
  spanClass := 0
  pointer := 0

def builtRectangularFalseSelectLocalEntries
    (shape : Cartesian.CartesianShape) :
    List SparseDenseFalseSelectLocatorEntry :=
  (List.range (builtRectangularFalseSelectLocalSlotCount shape)).map
    (builtRectangularFalseSelectLocalEntry shape)

theorem builtRectangularFalseSelectSuperEntries_length
    (shape : Cartesian.CartesianShape) :
    (builtRectangularFalseSelectSuperEntries shape).length =
      builtRectangularFalseSelectSuperSlotCount shape := by
  simp [builtRectangularFalseSelectSuperEntries]

theorem builtRectangularFalseSelectSuperEntries_get?
    (shape : Cartesian.CartesianShape) {superSlot : Nat}
    (hslot :
      superSlot < builtRectangularFalseSelectSuperSlotCount shape) :
    (builtRectangularFalseSelectSuperEntries shape)[superSlot]? =
      some (builtRectangularFalseSelectSuperEntry shape superSlot) := by
  simp [builtRectangularFalseSelectSuperEntries, List.getElem?_map,
    List.getElem?_range hslot]

theorem builtRectangularFalseSelectLocalEntries_length
    (shape : Cartesian.CartesianShape) :
    (builtRectangularFalseSelectLocalEntries shape).length =
      builtRectangularFalseSelectLocalSlotCount shape := by
  simp [builtRectangularFalseSelectLocalEntries]

theorem builtRectangularFalseSelectLocalEntries_get?
    (shape : Cartesian.CartesianShape) {globalLocalSlot : Nat}
    (hslot :
      globalLocalSlot < builtRectangularFalseSelectLocalSlotCount shape) :
    (builtRectangularFalseSelectLocalEntries shape)[globalLocalSlot]? =
      some (builtRectangularFalseSelectLocalEntry shape globalLocalSlot) := by
  simp [builtRectangularFalseSelectLocalEntries, List.getElem?_map,
    List.getElem?_range hslot]
...
theorem built_rectangular_local_entries_absolute_base_capacity_obstruction
    {shape : Cartesian.CartesianShape}
    (data : SparseDenseFalseSelectCloseData shape)
    (hlocalEntries :
      data.localEntries = builtRectangularFalseSelectLocalEntries shape)
    {globalLocalSlot : Nat}
    (hslot :
      globalLocalSlot <
        builtRectangularFalseSelectLocalSlotCount shape)
    (hneeds :
      2 ^ data.localFieldWidth <=
        builtRectangularFalseSelectLocalBaseOccurrence
          shape globalLocalSlot) :
    False := by
  have hbuilt :=
    builtRectangularFalseSelectLocalEntries_get?
      shape hslot
  have hget :
      data.localEntries[globalLocalSlot]? =
        some
          (builtRectangularFalseSelectLocalEntry
            shape globalLocalSlot) := by
    simpa [hlocalEntries] using hbuilt
  exact data.local_locator_baseOccurrence_capacity_obstruction
    hget hneeds

theorem built_rectangular_current_record_forces_local_absolute_bases_bounded
    {shape : Cartesian.CartesianShape}
    (data : SparseDenseFalseSelectCloseData shape)
    (hlocalEntries :
      data.localEntries = builtRectangularFalseSelectLocalEntries shape)
      ...
      data.localEntries[globalLocalSlot]? =
        some
          (builtRectangularFalseSelectLocalEntry
            shape globalLocalSlot) := by
    simpa [hlocalEntries] using hbuilt
  exact (data.local_locator_entry_fields_lt hget).1

theorem built_rectangular_local_entries_payload_capacity_obstruction
    {shape : Cartesian.CartesianShape}
    (data : SparseDenseFalseSelectCloseData shape)
    (hlocalEntries :
      data.localEntries = builtRectangularFalseSelectLocalEntries shape)
  ...
  have hrows :
      builtRectangularFalseSelectLocalSlotCount shape <=
        data.localEntries.length := by
    rw [hlocalEntries]
    rw [builtRectangularFalseSelectLocalEntries_length]
    exact Nat.le_refl _
  exact data.local_locator_rows_payload_capacity_obstruction
    hrows hneeds
...
structure RelativeSplitRectangularFalseSelectCloseData
    (shape : Cartesian.CartesianShape) where
  wordSize : Nat
  wordSize_eq :
    wordSize = sparseDenseFalseSelectWordBits shape
  ...
  sparseLocalRelativeEntries : List Nat
  superFieldWidth : Nat
  longSuperRelativeWidth : Nat
  localFieldWidth : Nat
  sparseLocalRelativeWidth : Nat
  superTable :
    FixedWidthSparseDenseFalseSelectDenseLocalEntryTable
      superEntries superFieldWidth
  ...
  localTable :
    FixedWidthSparseDenseFalseSelectDenseLocalEntryTable
      localEntries localFieldWidth
  sparseLocalRelativeTable :
    SuccinctSpace.FixedWidthNatTable
      sparseLocalRelativeEntries sparseLocalRelativeWidth
  bitWords : SuccinctSpace.BoundedPayloadWordStore shape.bpCode wordSize
  payload_length_le :
    (superTable.payload ++ longSuperRelativeTable.payload ++
      localTable.payload ++ sparseLocalRelativeTable.payload).length <=
        canonicalSparseDenseFalseSelectOverhead shape.size
  super_read_words_length_le_machine :
    superTable.ReadWordsLengthLeMachine shape.bpCode.length
  ...
  local_read_words_length_le_machine :
    localTable.ReadWordsLengthLeMachine shape.bpCode.length
  sparse_read_words_length_le_machine :
    forall {i : Nat} {word : List Bool},
      sparseLocalRelativeTable.store.words[i]? = some word ->
        word.length <=
          SuccinctRankProposal.machineWordBits shape.bpCode.length
  super_missing_exact :
    forall q,
    ...
    forall q super,
      ...
      relativeSplitFalseSelectEntryIsMarked super = true ->
        (longSuperRelativeEntries[
            relativeSplitFalseSelectLongExplicitSlot q superStride super]?).map
          (fun offset =>
  ...
  sparse_relative_exact :
    forall q super loc,
      ...
      relativeSplitFalseSelectEntryIsMarked loc = true ->
        (sparseLocalRelativeEntries[
            relativeSplitFalseSelectSparseExplicitSlot
              (relativeSplitFalseSelectLocalSlot q superStride
                localSlotsPerSuper localStride super)
              q localStride super loc]?).map
          (fun offset =>
...
namespace RelativeSplitRectangularFalseSelectCloseData

def payload
    {shape : Cartesian.CartesianShape}
    (data : RelativeSplitRectangularFalseSelectCloseData shape) :
    List Bool :=
  data.superTable.payload ++ data.longSuperRelativeTable.payload ++
    data.localTable.payload ++ data.sparseLocalRelativeTable.payload
...
def readWords
  ...
  ((((data.superTable.readWords ++
    ...
    data.sparseLocalRelativeTable.store.words.toList) ++
      data.bitWords.store.words.toList)

def queryOccurrence
    {shape : Cartesian.CartesianShape}
    (_data : RelativeSplitRectangularFalseSelectCloseData shape)
    (idx : Nat) : Nat :=
  Nat.min idx shape.bpCode.length
...
def selectCloseCosted
  ...
  Costed.bind
    (data.superTable.readCosted
      ...
      | none => Costed.pure none
      | some super =>
          if relativeSplitFalseSelectEntryIsMarked super = true then
            relativeOffsetReadCosted
              data.longSuperRelativeTable
              (relativeSplitFalseSelectEntryBasePosition
                data.wordSize super)
              (relativeSplitFalseSelectLongExplicitSlot
                q data.superStride super)
          else
            let localSlot :=
            ...
            Costed.bind (data.localTable.readCosted localSlot) fun loc? =>
              ...
              | none => Costed.pure none
              | some loc =>
                  if relativeSplitFalseSelectEntryIsMarked loc = true then
                    relativeOffsetReadCosted
                      data.sparseLocalRelativeTable
                      (relativeSplitFalseSelectLocalBasePosition
                        data.wordSize super loc)
                      (relativeSplitFalseSelectSparseExplicitSlot
                        localSlot q data.localStride super loc)
                  else
...
theorem selectCloseCosted_cost_le
    {shape : Cartesian.CartesianShape}
  (data : RelativeSplitRectangularFalseSelectCloseData shape) (idx : Nat) :
    (data.selectCloseCosted idx).cost <=
  ...
  | some super =>
      by_cases hlong :
          relativeSplitFalseSelectEntryIsMarked super = true
      · have hlongCost :=
          data.longSuperRelativeTable.readCosted_cost_le_one
            (relativeSplitFalseSelectLongExplicitSlot
              (Nat.min idx shape.bpCode.length) data.superStride super)
        simp [relativeOffsetReadCosted, Costed.bind, Costed.map,
          Costed.pure, hsuperValue, hlong] <;> omega
      · let localSlot :=
          relativeSplitFalseSelectLocalSlot
            (Nat.min idx shape.bpCode.length) data.superStride
            data.localSlotsPerSuper data.localStride super
        cases hlocalValue :
        ...
        | some loc =>
            by_cases hsparse :
                relativeSplitFalseSelectEntryIsMarked loc = true
            · have hsparseCost :=
                data.sparseLocalRelativeTable.readCosted_cost_le_one
                  (relativeSplitFalseSelectSparseExplicitSlot
                    localSlot (Nat.min idx shape.bpCode.length)
                    data.localStride super loc)
              simp [relativeOffsetReadCosted, Costed.bind, Costed.map,
                Costed.pure, hsuperValue, hlong, localSlot, hlocalValue,
                hsparse] <;> omega
            · have hdenseCost :=
                denseTwoWordFalseSelectCosted_cost_le_five
                  ...
                  (relativeSplitFalseSelectLocalBaseOccurrence super loc)
                  (Nat.min idx shape.bpCode.length)
              simp [Costed.bind, hsuperValue, hlong,
                localSlot, hlocalValue, hsparse] <;> omega
...
theorem selectCloseCosted_exact
  ...
  cases hsuper :
      data.superEntries[falseSelectSuperSlot q data.superStride]? with
  | none =>
      simp
      rw [<- hclamp]
      exact (data.super_missing_exact q hsuper).symm
  | some super =>
      by_cases hlong :
          relativeSplitFalseSelectEntryIsMarked super = true
      · simp [hlong, relativeOffsetReadCosted, Costed.erase_map,
          SuccinctSpace.FixedWidthNatTable.readCosted_erase]
        rw [<- hclamp]
        exact data.long_relative_exact q super hsuper hlong
      · have hlongFalse :
        ...
        | some loc =>
            ...
            · simp [hlong,
              ...
              have hlocal' :
                ...
                simpa [localSlot] using hlocal
              rw [hlocal']
              simp [hsparse, relativeOffsetReadCosted, Costed.erase_map,
                SuccinctSpace.FixedWidthNatTable.readCosted_erase]
              rw [<- hclamp]
              exact data.sparse_relative_exact q super loc hsuper
                hlongFalse hlocal' hsparse
            · have hsparseFalse :
...
theorem read_word_length_le_machine
  ...
  · rcases List.mem_append.mp hprefix0 with hprefix1 | hsparseMem
    · rcases List.mem_append.mp hprefix1 with hsuperOrLong | hlocalMem
      · rcases List.mem_append.mp hsuperOrLong with hsuperMem | hlongMem
        · exact data.superTable.read_word_length_le_machine
            data.super_read_words_length_le_machine hsuperMem
        · rcases (List.mem_iff_getElem?.mp hlongMem) with
            ⟨i, hgetList⟩
          have hget :
              data.longSuperRelativeTable.store.words[i]? = some word := by
            simpa [Array.getElem?_toList] using hgetList
    ...
    · rcases (List.mem_iff_getElem?.mp hsparseMem) with
        ⟨i, hgetList⟩
      have hget :
          data.sparseLocalRelativeTable.store.words[i]? = some word := by
        simpa [Array.getElem?_toList] using hgetList
      exact data.sparse_read_words_length_le_machine hget
  · exact Nat.le_trans (data.bitWords.word_length_le hbitsMem)
      data.wordSize_le_machine
      ...
      forall {word : List Bool},
        List.Mem word data.readWords ->
          word.length <=
            SuccinctRankProposal.machineWordBits shape.bpCode.length := by
  exact
    ⟨data.payload_length_le_canonical, data.selectCloseCosted_cost_le,
      data.selectCloseCosted_exact,
      fun {word} hmem => data.read_word_length_le_machine hmem⟩

end RelativeSplitRectangularFalseSelectCloseData
...
theorem relativeSplitRectangular_global_padded_sparse_payload_not_littleO
    {overhead : Nat -> Nat}
    (builder :
      forall shape : Cartesian.CartesianShape,
        RelativeSplitRectangularFalseSelectCloseData shape)
    (hpadded :
      forall shape : Cartesian.CartesianShape,
        builtRectangularFalseSelectLocalSlotCount shape *
            sparseDenseFalseSelectLocalStride shape <=
          (builder shape).sparseLocalRelativeEntries.length)
    (hcharged :
      forall shape : Cartesian.CartesianShape,
        (builder shape).sparseLocalRelativeTable.payload.length <=
          overhead shape.size)
    (hwidth :
      forall shape : Cartesian.CartesianShape,
        0 < (builder shape).sparseLocalRelativeWidth) :
    ¬ SuccinctSpace.LittleOLinear overhead := by
  apply not_littleOLinear_of_self_le
  intro n
  let shape := falseSelectRightSpine n
  ...
  have hpad :
      builtRectangularFalseSelectLocalSlotCount shape *
          sparseDenseFalseSelectLocalStride shape <=
        data.sparseLocalRelativeEntries.length := by
    simpa [data] using hpadded shape
  have hwidthPos : 0 < data.sparseLocalRelativeWidth := by
    simpa [data] using hwidth shape
  have hwidthOne : 1 <= data.sparseLocalRelativeWidth := by
    omega
  have hentriesPayload :
      data.sparseLocalRelativeEntries.length <=
        data.sparseLocalRelativeTable.payload.length := by
    calc
      data.sparseLocalRelativeEntries.length <=
          data.sparseLocalRelativeEntries.length *
            data.sparseLocalRelativeWidth := by
        simpa using
          Nat.mul_le_mul_left
            data.sparseLocalRelativeEntries.length hwidthOne
      _ = data.sparseLocalRelativeTable.payload.length := by
        exact data.sparseLocalRelativeTable.payload_length_eq.symm
  have hbudget :
      data.sparseLocalRelativeTable.payload.length <=
...
The rectangular sparse/dense record above still packs `baseOccurrence` and
`basePosition` into every high-frequency local locator row.  The theorem
`built_rectangular_current_record_forces_local_absolute_bases_bounded` names
the resulting pressure on the exact built rectangular rows: any inhabitant of
that record surface has to make every generated local absolute base fit in the
packed local field.

The surface below is the positive replacement checkpoint for this iteration.
