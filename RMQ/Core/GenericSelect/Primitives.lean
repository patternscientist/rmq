...
Generic `(target : Bool)` decode primitives over an arbitrary `bits : List Bool`.
These are the target-threaded versions of the older BP-specialized dense
two-word select helpers. Pure arithmetic slot/position helpers
(`selectSuperSlot`, the `relativeSplitSelect*` slot maths) are target-agnostic
...
the older BP-specialized dense two-word helper with the target bit made
explicit.
-/
  ...
  rankBefore_eq :
    rankBefore =
      RMQ.Succinct.rankPrefix target bits
        (selectDenseLocalFirstStart wordSize baseWordIndex)
  firstOffset_eq :
    firstOffset =
      basePosition - selectDenseLocalFirstStart wordSize baseWordIndex
  firstWordStart_readable :
    selectDenseLocalFirstStart wordSize baseWordIndex < bits.length
  rankBefore_le_query :
    rankBefore <= q
  first_branch_rank :
    q - baseOccurrence <
      ...
      q <
        RMQ.Succinct.rankPrefix target bits
          (selectDenseLocalSecondStart wordSize baseWordIndex)
  ...
  second_branch_rank :
    Not (q - baseOccurrence <
        denseLocalFirstCount
          target bits wordSize baseWordIndex firstOffset) ->
      RMQ.Succinct.rankPrefix target bits
          (selectDenseLocalSecondStart wordSize baseWordIndex) <= q /\
        q <
          RMQ.Succinct.rankPrefix target bits
            (selectDenseLocalSpanEnd wordSize baseWordIndex) /\
          selectDenseLocalSecondStart wordSize baseWordIndex <
            bits.length
...
def denseLocalSpanCertificate_of_payload_routing_facts
    ...
    {bitWords : SuccinctSpace.BoundedPayloadWordStore bits wordSize}
    {basePosition baseOccurrence q : Nat}
    (haligned : SelectAlignedBitWords bits wordSize bitWords)
    (hfacts :
  ...
  let firstStart :=
    selectDenseLocalFirstStart wordSize hfacts.baseWordIndex
  let secondStart :=
    selectDenseLocalSecondStart wordSize hfacts.baseWordIndex
  let spanEnd :=
    selectDenseLocalSpanEnd wordSize hfacts.baseWordIndex
  let firstWord :=
    selectDenseLocalFirstWord bits wordSize hfacts.baseWordIndex
  have hfirstReadAtBase :
    ...
    | intro word hread =>
        ...
        simpa [firstWord, selectDenseLocalFirstWord, firstStart,
          selectDenseLocalFirstStart, hword] using hread
  have hfirstRead :
  ...
  have hoffset :
    ...
    rw [<- hfacts.baseWordIndex_eq]
    simpa [firstStart, selectDenseLocalFirstStart] using
  ...
  have hfirstEnd :
    ...
    simp [secondStart, firstStart, selectDenseLocalSecondStart,
      selectDenseLocalFirstStart, Nat.succ_mul]
  have hspanEnd :
      spanEnd = secondStart + wordSize := by
    simp [spanEnd, secondStart, selectDenseLocalSpanEnd,
      selectDenseLocalSecondStart, Nat.add_assoc, Nat.succ_mul]
  refine {
    firstWord := firstWord
  ...
  · intro hchoose
    have hchoiceFacts :
      ...
      simpa [firstWord, denseLocalFirstCount,
        selectDenseLocalFirstWord, hoffset] using hchoose
    ...
    | intro pos hselect =>
        ...
        have hselectWord :
          ...
          simpa [firstWord, selectDenseLocalFirstWord, firstStart,
            selectDenseLocalFirstStart] using hlocal
        calc
          (RMQ.RAM.boolSelectInWord target firstWord
              (RMQ.RAM.boolRankPrefix target firstWord
                  (basePosition -
  ...
  · intro hnot
    have hnotFacts :
      ...
      exact hnot (by
        simpa [firstWord, denseLocalFirstCount,
          selectDenseLocalFirstWord, hoffset] using hchoice)
            ...
            | intro secondWord hsecondReadAtBase =>
                ...
                | intro pos hselect =>
                        ...
                        q - baseOccurrence -
                          ...
                          q -
                            RMQ.Succinct.rankPrefix target bits
                              secondStart := by
                      simpa [firstWord, denseLocalFirstCount,
                        selectDenseLocalFirstWord, secondStart,
                        hoffset] using
                    ...
                    have hselectWord :
                      ...
                      rw [hlocalOccurrence]
                      simpa [secondStart,
                        selectDenseLocalSecondStart] using hlocal
                    calc
                      (RMQ.RAM.boolSelectInWord target secondWord
                          (q - baseOccurrence -
                            (RMQ.RAM.boolRankPrefix target firstWord
...
def denseLocalPayloadRoutingFacts_of_selected_span
  ...
  let firstStart := selectDenseLocalFirstStart wordSize baseWordIndex
  let secondStart := selectDenseLocalSecondStart wordSize baseWordIndex
  let spanEnd := selectDenseLocalSpanEnd wordSize baseWordIndex
  ...
  have hfirstStartLeBase : firstStart <= basePosition := by
    simpa [firstStart, baseWordIndex, selectDenseLocalFirstStart] using
  ...
  have hbaseLtSecond : basePosition < secondStart := by
    ...
    simp [secondStart, baseWordIndex,
      selectDenseLocalSecondStart, Nat.succ_mul]
    omega
  ...
  have hfirstTotal :
      RMQ.RAM.boolRankPrefix target
          (selectDenseLocalFirstWord bits wordSize baseWordIndex)
          (selectDenseLocalFirstWord bits wordSize
  ...
  have hfirstOffsetRank :
    ...
    have hoffLen : firstOffset <=
      ...
      have hoffDrop : firstOffset <= (bits.drop firstStart).length := by
        simp [List.length_drop]
        omega
      simpa [selectDenseLocalFirstWord] using
...
def denseLocalEntrySelectCosted
  ...
  denseTwoWordSelectCosted target bitWords
    (sparseDenseSelectDenseLocalEntryBasePosition wordSize entry)
    entry.baseOccurrence q

theorem denseLocalEntrySelectCosted_cost_le_five
    (target : Bool) {bits : List Bool} {wordSize : Nat}
    (bitWords : SuccinctSpace.BoundedPayloadWordStore bits wordSize)
    (entry : SparseDenseSelectDenseLocalEntry) (q : Nat) :
...
the target-parametric relative-offset table used by sparse exception paths. -/
