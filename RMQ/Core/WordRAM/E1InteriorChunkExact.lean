...
(the summary group's segment assignment), not by this module, and stating
it here would guess a layout this module cannot check.
...
theorem hexact_of_segment_agrees
    ...
    (hagree : ∀ a, store.readWord? segment (base + a) =
  ...
  have haddr : base + i * chunkCount + j = base + (i * chunkCount + j) := by
    omega
  rw [haddr] at hw
  rw [hagree (i * chunkCount + j)] at hw
  rw [← hcount]
  exact hw
