...
`prologue + 2 * select + selectJoin + lca + rankJoin + rank + output` -- two
selects on every branch, and one each of the two connectives and the
close/LCA, rank and output stages, the skipped arms being covered by the same
bound as the run arms.  The `2 *` is not an assumption about the route: all
four branches charge `S.select left` and `S.select (right - 1)`, and the proof
is a case split that would fail if any branch charged a third.

**THIS STATEMENT WAS REVISED WHEN THE MACHINE-SIDE RECORD LANDED**
(DD-20260719-240).  As E1-LaneA7 wrote it the sum was
`prologue + 2 * select + lca + rank + output` against a record with a single
`lcaSkipped` field and no connective stages.  `E1WholeQueryCats.lean`'s record
subsequently gained `selectJoin` and `rankJoin` as named stages and SPLIT
`lcaSkipped` into `lcaSkippedLeftMiss` and `lcaSkippedRightMiss`
(DD-20260719-206, DD-20260719-208), because the machine charges the two
select-miss arms differently.  Both connectives are carried as their own
summands rather than folded into a neighbouring slot, for exactly the reason
the record names them separately: `select` and `lcaRun` each occur TWICE, so
absorbing a connective into either would give one function two meanings.  The
two skip arms take the `lca` bound each, which is where they sit in the log.

What this lane supplied for the `lca` slot is `10167`
(`closeLcaLegCats_length_le`); the remaining slots and the whole-query literal
are derived in `E1WholeQueryCostLiteral.lean`.
-/
theorem wholeQueryBranchCats_length_le_of (S : E1Query.WholeQueryStageCats)
    (left right : Nat) (br : SuccinctFinal.WholeQueryBranch)
    (p sel sj lca rj rank out : Nat)
    (hp : S.prologue.length ≤ p)
    (hsel : ∀ i, (S.select i).length ≤ sel)
    (hselJoin : S.selectJoin.length ≤ sj)
    (hlcaRun : ∀ i j, (S.lcaRun i j).length ≤ lca)
    (hlcaSkipLeft : S.lcaSkippedLeftMiss.length ≤ lca)
    (hlcaSkipRight : S.lcaSkippedRightMiss.length ≤ lca)
    (hrankJoin : S.rankJoin.length ≤ rj)
    ...
    (E1Query.wholeQueryBranchCats S left right br).length ≤
      p + sel + sj + sel + lca + rj + rank + out := by
  cases br with
  | leftSelectNone =>
...
theorem wholeQueryCats_length_le_of (S : E1Query.WholeQueryStageCats)
    ...
    (E1Query.wholeQueryCats S shape left right).length ≤
      p + sel + sj + sel + lca + rj + rank + out :=
  wholeQueryBranchCats_length_le_of S left right _ p sel sj lca rj rank out hp
    hsel hselJoin hlcaRun hlcaSkipLeft hlcaSkipRight hrankJoin hrankRun
    hrankSkip houtSome houtNone
